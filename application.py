    ...
    # if POST, validate and commit to database
        ...
        ## end validation
            ...
            try:
                ...
                db.execute(query, {"firstname": first_name, "lastname": last_name, "email": email, "password": generate_password_hash(password)})
...
def search():
    if request.method == "GET":
        ...
        try:
            query = text("SELECT * FROM books WHERE LOWER(isbn) LIKE :query OR LOWER(title) LIKE :query OR LOWER(author) LIKE :query")
            result = db.execute(query, {"query": "%" + query.lower() + "%"}).fetchall()
        except Exception as e:
            return render_template("error.html", message=e)
        if not result:
