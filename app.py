...
os.environ['DATABASE_URL'] = "postgresql://postgres:123456@localhost:5432/web1"
...
def register():
    ...
    # if POST, validate and commit to database
        ...
        ## end validation
            ...
            try:
                query = text("INSERT INTO users (firstname, lastname, email, password) VALUES (:firstname, :lastname, :email, :password)")
                db.execute(query, {"firstname": first_name, "lastname": last_name, "email": email, "password": password})
...
def login():
    ...
    if request.method == "POST":
        ...
        Q = db.execute(query, {"email": form_email}).fetchone()

        # User exists ?
        if Q is None:
            return render_template("error.html", message="User doesn't exists")
        # Valid password ?
        if Q.password != form_password:
...
def search():
    if request.method == "GET":
        ...
        try:
            input = text("SELECT * FROM books WHERE isbn LIKE :query OR title LIKE :query OR author LIKE :query")
            result = db.execute(input, {"query": "%" + str(query) + "%"}).fetchall()
        except Exception as e:
            return render_template("error.html", message=e)
        if not result:
...
def details(bookid):
     if request.method == "GET":
        ...
        query_book = text("SELECT * FROM books WHERE bookid = :bookid")
        result = db.execute(query_book, {"bookid": bookid}).fetchone()
        
        # Retrieve comments for the book from the database
        query_comment = text("SELECT u.firstname, u.lastname, u.email, r.rating, r.comment from reviews r JOIN users u ON u.userid=r.user_id WHERE book_id = :id")
        comment_list = db.execute(query_comment, {"id": bookid}).fetchall()
        if not result:
        ...
        query_user = text("SELECT * from reviews WHERE user_id = :user_id AND book_id = :book_id")
        user_reviewed_before = db.execute(query_user,  {"user_id": session["user_id"], "book_id": bookid}).fetchone()
        ...
        # try to commit to database, raise error if any
        try:
            commit = text("INSERT INTO reviews (user_id, book_id, rating, comment) VALUES (:user_id, :book_id, :rating, :comment)")
            db.execute(commit,{"user_id": session["user_id"], "book_id": bookid, "rating":user_rating, "comment": user_comment})
        except Exception as e:
            return render_template("error.html", message=e)
